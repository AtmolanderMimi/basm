...
You can probably get a better idea of the language overall just by reviewing the interpreter.
...
Myself, despite having already written an extemely similar interpreter in basm, being my own language,
has struggled with many bugs which were not mentioned in this book for brevity *and my ego a little*.
...
This issue caught up with me when writing this very chapter when testing the interpreter.
I didn't suspect that `IFEQ`, that I had written a while ago and worked well, would pollute cells.
...
I won't go over how to implement performance patches,
...
For example, you could lay bulked operator with this layout: `[operator][reccurence]`.
So, `[+][+][+][+]` could be represented as `[+][4]` saving both space and the time to get those cells.
(Saving space comes with saving time, as, the bigger the array is, the longer it will take to index)
...
While you are at it storing items of two cells wide in an indexable array,
you could also implement indexable arrays of 2 cell big elements.
Firstly, that would save you doing two gets/sets for each element.
...
You won't be able to use the same meta-instructions to get/set the program and memory arrays,
...
In the advent you would want to make the bf interpreter run the bf interpreter,
you will need to find a way to move from 0 to the operating memory segment without using `program_capacity` times `>`.
Because, that would mean that your interpreter is always bigger than its program array.
This is of course something you would want to avoid if you were to want to run it on itself.
...
or just don't use an interpreter an get a compiler.
While I believe the built-in interpreter is capable of running an helping somewhat with debugging via
its dump feature (with flag `-d`), I am not saying its the best for performance in no way.

If you want to get just the transpiled basm code, use `basm compile` rather than `basm run`.
You can then use that bf in another bf interpreter / compiler. (What I mean by "compiler", is something that compiles to machine code)
